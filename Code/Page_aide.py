...
def aidegénéral(): # Des aides générales pour tout débutant ou juste n'importe qui.
    ...
    with idéequestion(text='Comment est-je accéder à la page aide ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
    ...
    with idéequestion(text='Comment puis-je accéder à d\'autres pages ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
...
def aideprofile(): # Explication de comment la page pour le profile et le profile en général fonctionne
    ...
    with idéequestion(text='Comment puis-je accéder à mon propre profile ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
    ...
    with idéequestion(text='Quelles sont les modifications que je peux faire à mon profil et comment ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
...
def quinoussommes(): # Explication de qui on est, ce qu'on fait, et pourquoi le site, etc...
    ...
    with idéequestion(text='Qui sommes nous ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
    ...
    with idéequestion(text='Pourquoi un site? Et pourquoi local ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
    ...
    with idéequestion(text='D\'ou vient cette idée du site ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
    ...
    with idéequestion(text='Quelles étaient vos inspirations ?', group='group').style(f'color: #ffffff; background-color: {Layout.c2act}; border-radius: 5px; {police3}').classes('w-full text-2xl'):
...
def titre(texte): # Le titre de la page dans laquelle l'utilisateur se trouve
    ui.label(texte).style(f'color: {Layout.ctexte}').classes('text-4xl font-bold self-center').style(f'{police3}')
    ui.separator()
    
def synops(texte): # Un résumé de pourquoi l'utilisateur se trouverais sur cette page et ce qu'il y a
    ui.label(texte).style(f'color: {Layout.ctexte}').classes('text-2xl text-center').style(f'{police5}')
    ui.separator()
