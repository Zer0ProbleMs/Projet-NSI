...
def aidegénéral(): # Des aides générales pour tout débutant ou juste n'importe qui.
    ...
    with idéequestion(text='Comment est-je accéder à la page aide ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
    ...
    with idéequestion(text='Comment puis-je accéder à d\'autres pages ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
...
def aideprofile(): # Explication de comment la page pour le profile et le profile en général fonctionne
    ...
    with idéequestion(text='Comment puis-je accéder à mon propre profile ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
    ...
    with idéequestion(text='Quelles sont les modifications que je peux faire à mon profil et comment ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
...
def quinoussommes(): # Explication de qui on est, ce qu'on fait, et pourquoi le site, etc...
    ...
    with idéequestion(text='Qui sommes nous ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
    ...
    with idéequestion(text='Pourquoi un site? Et pourquoi local ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
    ...
    with idéequestion(text='D\'ou vient cette idée du site ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
    ...
    with idéequestion(text='Quelles étaient vos inspirations ?', group='group').style(f'color: #ffffff; background-color: {violetmoyen}; border-radius: 5px').classes('w-full text-2xl font-medium'):
...
def idéesynops(texte): # Pas obligatoir, mais pareil que le synops principal, mais se trouve cependant dans "l'idée question"
    ui.label(texte).classes('text-xl text-center font-medium')
    ui.separator()

def idée(texte): # Explication compréhensible des réponses au questions
    ui.label("> " + texte).classes('text-xl font-light')
