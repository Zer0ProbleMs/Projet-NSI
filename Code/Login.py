...
def login_page():
    ...
    with ui.card().classes('absolute-center').style(
        f'background-color: {Layout.fondsecondaire}; '
        f'border-radius: 15px; '
        f'box-shadow: 0px 0px 20px {Layout.couleurcontour}; '
        f'padding: 3rem; '
        f'width: 400px; '
        f'min-height: 350px;'
    ):
        ...
        username = ui.input('Pseudo').style(
            f'background-color: {Layout.couleurbouton}; border-radius: 8px; width: 100%'
        )
        password = ui.input('Mot de passe', password=True, password_toggle_button=True).style(
            f'background-color: {Layout.couleurbouton}; border-radius: 8px; width: 100%'
        )
